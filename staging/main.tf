...
terraform {
  backend "s3" {
    bucket = "cmutzlitz-platform-engineering-terraform-state"
...
variable "confluent_cloud_environment_name" {
  default = "cmutzlitz-gitops"
}

variable "confluent_cloud_cluster_name" {
  default = "cmutzlitz-gitops-cluster003"
}
...
resource "confluent_service_account" "cmplatform-manager" {
  display_name = "cmplatform-manager-${confluent_environment.env.display_name}"
  description  = "Service account to manage the platform"
}

# The 'platform-manager' account is a cloud cluster admin.
resource "confluent_role_binding" "cmplatform-manager-kafka-cluster-admin" {
  principal   = "User:${confluent_service_account.cmplatform-manager.id}"
  role_name   = "CloudClusterAdmin"
  crn_pattern = confluent_kafka_cluster.standard.rbac_crn
}

resource "confluent_api_key" "cmplatform-manager-kafka-api-key" {
  display_name = "cmplatform-manager-kafka-api-key"
  description  = "Kafka API Key that is owned by 'platform-manager' service account"
  owner {
    id          = confluent_service_account.cmplatform-manager.id
    api_version = confluent_service_account.cmplatform-manager.api_version
    kind        = confluent_service_account.cmplatform-manager.kind
  }
  ...
  depends_on = [
    confluent_role_binding.cmplatform-manager-kafka-cluster-admin
  ]
}